...
describe("probability calibration desk", () => {
  ...
  it("survives SQLite restart without duplicating observations or milestone snapshots", () => {
    ...
    expect(secondDesk.projection()).toMatchObject({
      ...
      storage: {
        bounds: { mode: "SQLITE_WAL", durable: true, schemaVersion: 41 },
        observations: { mode: "SQLITE_WAL", durable: true, schemaVersion: 41 },
        snapshots: { mode: "SQLITE_WAL", durable: true, schemaVersion: 41 },
      },
    });
